...
stage=0 			# sets the stage of processing
stop_stage=0 		# sets stage to stop processing, if 2 the program will nog execute stage 2
do_decoding="yes" 	#whether to do the decoding 
do_training="yes" 	#whether to do the training
do_to_end=
...
skip_data_prep= 				   # whether to skip the data preparation
test_run= 					   # if set to anything only show  parameters

. ./utils/parse_options.sh # helper script to parse command line arguments
...
#set skip_data_prep to skip anything before dataprep
if [ ! -z $skip_data_prep ]; then
	if [ $stage -le "2" ]; then
		echo "skipping data preperation: changing start stage to 3"
		stage=3
	else 
		echo "skipping data preperation: start stage $stage is already after data preperation"
	fi
fi

if [ $do_to_end ]; then
	echo doing everything, setting stop stage to 12
	stop_stage=12
fi

echo "running script with data directories:"
echo ${directories[@]}
echo "start stage: $stage, stops before stage: $stop_stage"
...
if [ $stage -le 0 -a $stop_stage -gt 0 ]; then
  printf "\n\n--- stage one starting --- data preparation\n\n"
  echo $(date)
  printf "\n\n--- data prep\n\n"
  local/council_data_prep.sh --corpus $council_corpus --data_dir $data_dir || exit 1;
fi

if [ $stage -le 1 -a $stop_stage -gt 1 ]; then
  printf "\n\n--- stage one starting --- dict - lang - lm preparation\n\n"
  printf "\n\n--- dict prep\n\n"
  local/council_dict_prep.sh --corpus $council_corpus || exit 1;
...
if [ $stage -le 3 -a $stop_stage -gt 3 ]; then
  ### Monophone
	...
	if [ $do_training == "yes" ]; then
		  ...
		  echo $(date)
		  steps/train_mono.sh --nj $train_nj --cmd "$train_cmd" data/$train \
		data/lang $exp/mono || exit 1;
		  echo "Mono training done."
	fi

	if [ $do_decoding == "yes" ]; then
...
if [ $stage -le 6 -a $stop_stage -gt 6 ]; then
  ...
  steps/decode_fmllr.sh --nj $decode_nj --cmd "$decode_cmd" $exp/tri3/graph data/${test}_nl $exp/tri3/decode_test_nl || exit 1;
  steps/decode_fmllr.sh --nj $decode_nj --cmd "$decode_cmd" $exp/tri3/graph data/${test}_fr $exp/tri3/decode_test_fr || exit 1;
  steps/decode_fmllr.sh --nj $decode_nj --cmd "$decode_cmd" $exp/tri3/graph data/${test}_mx $exp/tri3/decode_test_mx || exit 1;
  steps/decode_fmllr.sh --nj $decode_nj --cmd "$decode_cmd" $exp/tri3/graph data/${dev}_nl $exp/tri3/decode_dev_nl || exit 1;
  steps/decode_fmllr.sh --nj $decode_nj --cmd "$decode_cmd" $exp/tri3/graph data/${dev}_fr $exp/tri3/decode_dev_fr || exit 1;
  steps/decode_fmllr.sh --nj $decode_nj --cmd "$decode_cmd" $exp/tri3/graph data/${dev}_mx $exp/tri3/decode_dev_mx || exit 1;
...
if [ $stage -le 7 -a $stop_stage -gt 7 ]; then
  ...
  echo $(date)
     local/nnet3/run_ivector_common.sh --exp $exp --train_set $train --dev $dev --test $test --data_dir $data_dir
...
if [ $stage -le 8 -a $stop_stage -gt 8 ]; then
  ...
  echo $(date)
	local/chain/run_tdnn.sh --exp $exp --train_set $train --dev $dev --test $test --data_dir $data_dir
