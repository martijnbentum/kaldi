...
if [ "$prefix" = '' ]; then
	printf "using no prefix \n\n"
else
	printf "using prefix $prefix \n\n"
fi

for x in $train $dev $dev'_nl' $dev'_fr' $dev'_mx' $test $test'_nl' $test'_fr' $test'_mx' $exp; do
	echo $x
done
...
if [ $stage -le 0 -a $stop_stage -gt 0 ]; then
  ...
  echo $(date)
  printf "\n\n--- data prep\n\n"
  local/council_data_prep.sh $council_corpus || exit 1;
fi
...
if [ $stage -le 2 -a $stop_stage -gt 2 ]; then
  ...
  for x in $train $dev $dev'_nl' $dev'_fr' $dev'_mx' $test $test'_nl' $test'_fr' $test'_mx'; do
...
if [ $stage -le 7 -a $stop_stage -gt 7 ]; then
  ...
  echo $(date)
     local/nnet3/run_ivector_common.sh --exp $exp --train_set $train
...
if [ $stage -le 8 -a $stop_stage -gt 8 ]; then
  ...
  echo $(date)
	local/chain/run_tdnn.sh --exp $exp --train_set $train --dev $dev --test $test
